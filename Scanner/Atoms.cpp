...
std::string MemoryOperand::toString() const {
    return _SymbolTable->operator[](_index)._name;
}
...
BinaryOpAtom::BinaryOpAtom(const std::string &name, std::shared_ptr <RValue> left, std::shared_ptr <RValue> right,
...
UnaryOpAtom::UnaryOpAtom(const std::string &name, std::shared_ptr <RValue> operand,
                         std::shared_ptr <MemoryOperand> result) {
    _name = name;
    _operand = operand;
    _result = result;
...
std::string BinaryOpAtom::toString() const {
    std::string tmp =
            "(" + _name + ", " + _left->toString() + ", " + _right->toString() + ", " + _result->toString() + ")";
    return tmp;
}
...
std::string ConditionalJumpAtom::toString() const {
    std::string tmp =
            "(" + _condition + ", " + _left->toString() + ", " + _right->toString() + ", " + _label->toString() + ")";
    return tmp;
}
